...
def insert_tree(x, tree):
    ...
    if x <= entry(tree):
        ...
        if is_empty_tree(left):
            left_tree = build_tree(x, make_empty_tree(), make_empty_tree())
        else:
            left_tree = insert_tree(x, left)
        return build_tree(entry(tree), left_tree, right_branch(tree))
    elif x > entry(tree):
        right = right_branch(tree)
        if is_empty_tree(right):
            right_tree = build_tree(x, make_empty_tree(), make_empty_tree())
        else:
            right_tree = insert_tree(x, right)
        return build_tree(entry(tree), left_branch(tree), right_tree)
